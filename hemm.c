...
int getSyndrome(char* message, int part, int begin)
{
	...
	{
		sum = 0;	// pos_p - используется дважды, чтобы не использовать еще одну переменную	
		...
		for(int j = temp; j<part; j+=(i+1)*2)
		{
			for(int k = 0; k<=i; k++) // подгруппа контролируемых битов
			{
				...
				{				
					//printf("%i\n", ((j + k + begin) % byte_size));
					if((message[(j + k + begin) / byte_size] & (mask >> ((j + k + begin) % byte_size))) != 0)
					{
...
char* hemming(char* part, int size, int pos_p)
{
	...
	int temp, kbits=countKbits(size); // количество контрольных бит
	int enc_size = size+kbits;
	int part_size = (size+kbits+1)/byte_size+((size+kbits+1)%byte_size>0?1:0); // размер закодированной части в байтах
	char buf, *result = (char*)malloc(part_size), *buff;
	int r_offset=0; // pos_p - позиция в part, r_offset - смещение для вставки в result
	//printf("%i\n", part_size);
	for(int i=0; i<part_size;i++) result[i]=0; //обнуление массива
	// Заполнение массива исходными данными
	for(int i=0; i<kbits; i++)
	{
		temp = pow(2,i); // позиция контрольного бита
		for(int pos_r = temp; pos_r<temp*2-1 && pos_r<enc_size; ++pos_r) // от текущего до следующего контрольного бита
		{
	...
	if(pos_p%2!=0) result[0] |= mask; // установка контрольного бита
	return result;
}
...
char* setCode(char* message, int part){
	...
	for(int i=0; i<res_size; i++) result[i]=0;
...
char* getMessage(char* code, int part){
	...
	int res_size = (count_parts*part%byte_size>0)? count_parts*part/byte_size+1 : count_parts*part/byte_size;
	char* result = (char*)malloc(res_size);
	for(int i=0; i<res_size; i++) result[i]=0;
...
int main(int argc, char** argv){
	setlocale(0,"");
	char* ans = setCode("Hello!", 8); // кодирование
	unsigned char t = 0x80 >> 5; 
	ans[0] ^= t; // добавление ошибки
	ans[3] ^= t >>1; // добавление ошибки
	char* temp = getMessage(ans, 8); // декодирование
	printf("\n%s\n", temp);
	free(ans);	
	free(temp);	
	return 0;
